...
class Plugin:
    ...
    compliance_frameworks: List[str] = None  # Supported compliance frameworks
...
class PluginRegistry:
    ...
    def register(self, plugin: Plugin) -> None:
        ...
        if plugin.service in self._plugins:
            self.logger.warning(f"Plugin for {plugin.service} already registered, overwriting")
        
        self._plugins[plugin.service] = plugin
        self.logger.info(f"Registered plugin: {plugin.name} ({plugin.service})")
    ...
    def get_required_permissions(self, services: Optional[List[str]] = None) -> List[str]:
        """
        Get all required AWS permissions for specified services.
        
        Args:
            services: List of service names. If None, returns permissions for all services.
    ...
    def get_compliance_frameworks(self, services: Optional[List[str]] = None) -> List[str]:
        """
        Get all supported compliance frameworks for specified services.
        
        Args:
            services: List of service names. If None, returns frameworks for all services.
    ...
    async def execute_plugin(self, service: str, context: AuditContext) -> List[Finding]:
        """
    ...
    def validate_plugins(self) -> Dict[str, List[str]]:
        ...
        for service, plugin in self._plugins.items():
            ...
            if not plugin.required_permissions:
                errors.append("Plugin required_permissions should not be empty")
            ...
            if plugin.scan_function:
                ...
                if len(sig.parameters) != 1:
                    errors.append("Scan function must accept exactly one parameter (AuditContext)")
