...
Enhanced for multi-account environments with cross-account support.
"""
...
@dataclass
class AuditContext:
    """Manages AWS credentials for multi-account environments."""
    ...
    _async_client_manager: Optional[AsyncClientManager] = field(
    ...
    def __post_init__(self):
        ...
        logger.info(
            f"AuditContext initialized for {self.account_id} in {self.regions}"
        )
    ...
    def _validate_configuration(self):
        ...
        if self.delegated_admin_account and not ACCOUNT_ID_PATTERN.match(
            self.delegated_admin_account
        ):
            raise ValueError(
                f"Invalid delegated admin account: "
                f"{self.delegated_admin_account}"
            )
    ...
    async def get_async_client(
        ...
        region = region_name or self.region
        return await self._async_client_manager.get_client(
            service_name, region
        )

    def assume_role(
        self, role_arn: str, session_name: Optional[str] = None
    ) -> "AuditContext":
