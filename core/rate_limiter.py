...
def get_rate_limiter(
    service: str, custom_limit: Optional[float] = None
) -> Throttler:
    """Get or create a rate limiter for the specified service."""
    if service not in _rate_limiters:
        limit = custom_limit or DEFAULT_RATE_LIMITS.get(
            service, DEFAULT_RATE_LIMITS["default"]
        )
...
async def rate_limited_call(
    service: str, coro, custom_limit: Optional[float] = None
):
